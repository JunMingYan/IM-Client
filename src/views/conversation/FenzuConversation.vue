  ...
  export default {
    ...
    computed: {
      ...
      fenZu() { // 用户的分组列表
        ...
        return this.newFenZu ? Object.keys(this.friendGroups || {}).concat(this.newFenZu) : Object.keys(this.friendGroups || {})
      },
      beiZhu() { // 备注map
      ...
      outcomeConversation() { // 根据分组来分类不同的好友
        ...
        const fenZuMap = this.friendGroups // {分组1: [id1, id2, ...], 分组2: [id3, id4, ...]}
    ...
    methods: {
      ...
      async getMyFriends() {
        ...
        const myFriendsList = data.data.myFriendsList
        if (data.code === 2000 && (100 <= status <= 400) && myFriendsList) {
          ...
          //重新更新一下分组人数的统计
          const uInfo = await this.$http.getUserInfo(this.userInfo.userID)
      ...
      async deleteFenZuItem() {
        ...
        const params = {fenZuName: this.currClickFenZu, userID: this.userInfo.userID}
        const {data} = await this.$http.deleteFenZu(params)
        if (data.code === 2000) {
          //重新更新用户信息
          this.$store.dispatch('user/GROUPS', data.data.friendGroups)
