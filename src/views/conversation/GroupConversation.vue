  ...
  <div class="group-conversation-list">
    <el-collapse v-model="activeFenZu">
      <el-collapse-item
        ...
        :name="item.category">
        <conversation-item
          v-for="item1 in groupCategoryMap[item.category]"
          :key="item1.groupID"
          :conversationInfo="item1"
          type="fenzu"
          @click.native="changeCurrentConversation(item1)"/>
      </el-collapse-item>
    </el-collapse>
    <div class="create">
      <i
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      myHolderGroup() {
        return this.conversationList.filter(item => item.role == 2)
      },
      myManagerGroup() {
        return this.conversationList.filter(item => item.role == 1)
      },
      myJoinGroup() {
        return this.conversationList.filter(item => item.role == 0)
      },
      groupCategoryMap() {
        return {
