<template>
  <div
    ...
    <el-avatar
      ...
      class="avatar"
      size="large"
      :src="messageitem.senderID === userInfo.userID ? IMG_URL + userInfo.photo : IMG_URL + messageitem.senderAvatar"
      @error="() => true">
    ...
    <div
      ...
      <span
        ...
        :style="messageitem.senderID === userInfo.userID ? {'flex-direction': 'row-reverse'} : {}">
...
<!--                    </router-link>-->
        ...
        <span
          class="item time"
          :style="messageitem.senderID === userInfo.userID ? {'float': 'right'} : {}">{{messageitem.sendTime | formatDateToZH}}</span>
      </span>
      <div
        ...
        <span
          ...
          <el-tooltip class="item" effect="dark" :content="isAllRead ? '已读' : '未读'" placement="top">
            <i v-if="!currentConversation.isGroup && messageitem.senderID === userInfo.userID"
...
<script>
  ...
  export default {
    ...
    methods: {
      async lookUserInfo() {
        if (this.messageitem.senderID === this.userInfo.userID) return
        let friendInfo = {}
    ...
    computed: {
      ...
      contentClassName() {
        ...
        if (this.messageitem.messageType === 'sys') {
          ...
          res = this.messageitem.senderID === this.userInfo.userID ? 'normal-content isme' : 'normal-content'
        }
        return res
      },
      messageItemComStyle() {
        ...
        } else if (this.messageitem.senderID === this.userInfo.userID) {
      ...
      messageWraperStyle() {
        let res = {}
        if (this.messageitem.messageType === MSG_TYPES.img && this.messageitem.senderID === this.userInfo.userID) {
          res = {'float': 'right'}
        } else if (this.messageitem.senderID === this.userInfo.userID) {
      ...
      isAllRead() { // 两个用户是否都阅读了消息
        ...
        if(this.currentConversation.conversationType  == conversationTypes.friend){
          ...
          // const flag1 = isReadUserArr.includes(intArr[0]) && isReadUserArr.includes(intArr[1])
          // const flag2 = this.userIsReadMsg[this.messageitem.roomID] || false
          // return this.messageitem.sendTime < this.lastEnterTime || flag1 || flag2
          return this.messageitem.sendTime < this.lastEnterTime || this.messageitem.read
        }
      },
      device() {
