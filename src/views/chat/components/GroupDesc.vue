    ...
    methods: {
      async fetchGroupInfo() {
        if (this.currentConversation.conversationType === 'GROUP') {
          ...
          const groupId = this.currentConversation.groupInfo.id
          const {data} = await this.$http.getGroupInfo(groupId)
          const groupUserList = data.data.groupUserList
          // console.log("返回的用户信息为：", groupUserList)
          if (data.code === 2000) {
            this.groupMembers = groupUserList
          }
        }
      }
    },
