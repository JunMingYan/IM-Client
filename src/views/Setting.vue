<template>
  <div class="setting-page" :style="device === 'Mobile' ? {width: '95%'} : {}">
    <div class="header">
      ...
      <div class="info-list">
        <div class="info-item">MessagerId：{{userInfo.userID}}</div>
        <div class="info-item">账号：{{userInfo.userName}}</div>
          ...
          <li
            ...
            <div class="action">
              ...
              <div class="oper" v-show="isModifying[item]">
                ...
                <span class="operation-text__danger" @click="setModify(item, false)">取消</span>
              </div>
            </div>
          </li>
        </ul>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async updateUserPwd() {
        const params = Object.assign({}, this.pwdSetting, {userID: this.userInfo.userID})
        const {data} = await this.$http.updateUserPwd(params)
        if (data.code === 2000) {
          //重置为空
          this.pwdSetting.newPwd = this.pwdSetting.oldPwd = this.pwdSetting.reNewPwd = ''
