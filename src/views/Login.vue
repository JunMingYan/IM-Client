    ...
    <div class="wrapper hor-ver-center" :style="device === 'Mobile' ? {width: '90%'}:{}">
      <el-form class="login-form" v-if="isLoginState">
        ...
        <el-form-item class="cv-code">
          ...
          <img width="120" height="40" v-show="!cvCodeIng" :src='this.cvCode'  @click="getCVCode"/>
    ...
    methods: {
      login() {
        ...
        this.$http.login(params).then(res => {
          ...
          if (res.status === 200 && code === 1001) {
            this.$message.success('登录成功！')
            setCookie(data.token)
            this.$store.dispatch('user/LOGIN', data.userInfo)
            this.$store.dispatch('user/GROUPS', data.friendGroups)
            this.$store.dispatch('user/NOTES', data.friendNotes)
      ...
      getCVCode() { // 获取验证码
        ...
        this.$http.getCVCode().then(res => {
          this.cvCode = "data:image/png;base64,"  + res.data.data
