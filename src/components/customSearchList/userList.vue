<template>
  <div class="custom-search-list-com">
    <div class="wrapper" v-for="item in searchList" :key="item.uid">
      ...
      <div class="operation">
        <el-button
          v-if="!(friends || []).includes(item.userID)"
          type="primary"
          size="small"
          icon="el-icon-plus"
  ...
  export default {
    ...
    methods: {
      ...
      async sendApply() {
        ...
        // const val = {
        //   roomId: validateSysUsr.userId + '-' + this.selectItem.uid,
        ...
        const message = {
          ...
          time: new Date(), // fromatTime(),
          additionMessage: this.additionMessage,
          status: 0,
          validateType: 0
        }
        await this.$http.getValidateMessage({ // 要用同步请求，否则会出错
          senderID: validateSysUsr.userId,
          receiverID:this.selectItem.uid,
          status: 0,  //未处理状态
          validateType: 0
        }).then(res => {
          ...
          if (validateMessage && validateMessage.roomId) {
            ...
            this.$http.sendValidateMessage(message).then(res => {
              if (res.data.code != 2000){
                this.$alert('验证消息发送失败！' + res.data.message, '提示', {
                  confirmButtonText: '确定',
                  type: 'success'
                })
              }
