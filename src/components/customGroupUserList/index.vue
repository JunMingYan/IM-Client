  ...
  <div class="custom-group-user-list-com">
    ...
    <div class="user-list">
      <div class="user-item" v-for="item in outcomeUserList" :key="item.ID">
        <el-avatar
          :class="(onlineUser || []).includes(item.userID) ? '' : 'offline'"
          :size="20"
        ...
        <div class="user-detail">
          <span class="normal-font detail-item ellipsis">
            {{item.beiZhu ? item.beiZhu : item.userName}}
          </span>
    ...
    computed: {
      ...
      groupOnlineUser() {
        return this.userList.filter(item => {
          ...
          return (this.onlineUser || []).includes(item.userID)
        })
      },
      ...
      outcomeUserList() {
        ...
        return userList.map(item => {
          ...
          if (user.userID != item.userID){
            item.beiZhu = this.beiZhu[item.userID] ? this.beiZhu[item.userID] : ''
          }
          return item
        }).sort((a, b) => {
